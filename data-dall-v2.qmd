...
# Data Dalliances (WIP) {#sec-data-dall}
...
```{r dgp, echo = FALSE, out.width = '90%', fig.align = 'center', fig.cap = 'A schematic of the data production process'}
#| label: fig-dgp
...
No inclusion/exclusion decision here is *incorrect*, per say, but if the data producers' choices don't match the analyst's understandings, a resulting analysis can be misleading.

For example, an analyst might seek out a `logins` table in order to calculate the rate of successful website logins. 
We might agree that this rate should be computed as the sum of successful events over the total attempts. 
But what actually *is* a success? What *is* an attempt?

Suppose two users attempt to login to their account, and ultimately, one succeeds in accessing their private information and the other doesn't. 
The analyst would probably hope to compute and report a 50% login success rate. 
However, depending on how the data is represented, they could quite easily compute nearly any value from 0% to 100%. 

Consider what types of events might be logged: 
...
These different situations are illustrated in @fig-login-rate, and their calculations are shown in @tbl-login-rate.
...
(Note that we did not explicitly "simulate" data here, but the workflow is largely the same. 
We imagine a real-world process, how that might translate into a digital representation, and created a numerical example to understand the implications. 
Not all simulations require fancy code; sometimes paper-and-pencil thought experiment works just fine.)
...
Any misunderstanding in precisely what that definition is can have massive impact on the perceived data quality; 
"per event" data will appear heavily duplicated if it is assumed to be "per session" data.

In some cases, this could be obvious to detect. 
If the `logins` table had a field `step_in_the_login_process` with values like `"MFA"` or `"Captcha"`, an analyst would be immediately confronted with this inclusion-exclusion choice. 
However, if that same information was encoded in fields like `sourceid` and `processid` with untuitive values (e.g. `"001"` means MFA, `"002"` means Captcha), it could be easy to misunderstand this data without high-quality documentation or a discussion with a subject-matter expert.

### What Doesn't Make a Record (Row)

It's important to think about what *does* count (or get logged) in our data. 
It's equally important to understand what systemically does not generate a record. 

Returning to our example, consider other users who have the intent or desire to login but cannot find the login page, or users who load the login page but never click a button because they know that they have forgotten their password and see no way to request it. 
These corner cases might be small in number -- or they might be large. 
That's precisely the problem - if we are not capturing data on these records, we cannot tell.
Regardless of their magnitude, depending on the question we are analyzing, these corner cases could be some of he most critical and informative observations to know about.
For example, they could uncover flaws in the design of our login page's user interface. 

It's hard to *computationally* validate what data doesn't exist, so *conceptual* data validation is critical.

### Records versus Keys

The preceding discussion of what types of real-world observations will or will not generate records is related to but distinct from another important concept from the world of relational databases: **primary keys**. 
...
however, that is no guarantee that they *must* contain every session and timestamp in the universe of relevant events.
...
Superficially, this task seems easier: after all, each variable has a name which hopefully includes some semantic information. However, this information often provides a false sense of security. 
...
You might find a table of orders with a field called `amt_spend`. 
But what might this mean?
That depends on *how* and for *what purpose* the data was collected.
...
Reading variable names alone is not enough.
It is critical to understand what each variable *actually* means. 
...
As we've seen, concepts like "spend" seem deceptively simple but can still be ambiguous.

## Extraction & Loading

**Data extraction** refers to the process of removing data from the system in which it was collected, and **data loading** refers to moving the extracted data into another system for further processing and analysis.

Data extraction and loading can occur in many different ways.
Such processes might be: 
...
Data movement may sound like one of the simplest and most rote parts of data production.
However, the superficially simple ac of loading data and updating data over time can introduce a variety of risks to data **completeness**, **uniqueness**, and **timeliness** which require different strategies to detect. 
These principles suggest that records should exist in our dataset once and only once, 
but the relating of many haphazard loading processes means that data appears sometimes between zero and a handful of times. 
...
Similarly, understanding how our null data is collected has significant implications for how we subsequently process it. We will discuss this more in @sec-comp-quan (Computational Quandaries).
